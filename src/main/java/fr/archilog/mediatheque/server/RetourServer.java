...
public class RetourServer {
    ...
    public void demarrer() {
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {
            System.out.println("[RetourServer] Démarré sur le port " + PORT);
            while (true) {
                try (Socket clientSocket = serverSocket.accept();
                    ...
                    Response welcome = new Response(RequestStatus.SUCCESS, "Bienvenue sur le service de retour");
                    out.println(welcome);
                    System.out.println("[RetourServer] Message d'accueil envoyé: " + welcome);
                    ...
                    if (numAbonneStr == null || numAbonneStr.isEmpty()) {
                        ...
                        out.println(error);
                        System.out.println("[RetourServer] Erreur: " + error);
                        return;
                    }
                    ...
                    out.println(confirm);
                    System.out.println("[RetourServer] Confirmation: " + confirm);
                    ...
                    // 7. Effectuer le retour
                    try {
                        document.retourner();
                        Response success = new Response(RequestStatus.SUCCESS, "Retour effectué avec succès");
                        out.println(success);
                        System.out.println("[RetourServer] Succès: " + success);
                    } catch (Exception e) {
                        Response error = new Response(RequestStatus.FAILURE, "Erreur lors du retour: " + e.getMessage());
                        out.println(error);
                        System.out.println("[RetourServer] Erreur: " + error);
                    }
                    ...
                    System.out.println("[RetourServer] Confirmation client reçue: " + confirmation);
                }
            }
        } catch (IOException e) {
            System.err.println("[RetourServer] Erreur: " + e.getMessage());
        }
    }

    public static void main(String[] args) {
        new RetourServer().demarrer();
    }
} 