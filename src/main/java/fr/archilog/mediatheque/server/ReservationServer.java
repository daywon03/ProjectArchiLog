...
public class ReservationServer {
    ...
    public void demarrer() {
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {
            System.out.println("[ReservationServer] Démarré sur le port " + PORT);
            while (true) {
                try (Socket clientSocket = serverSocket.accept();
                    ...
                    Response welcome = new Response(RequestStatus.SUCCESS, "Bienvenue sur le service de réservation");
                    out.println(welcome);
                    System.out.println("[ReservationServer] Message d'accueil envoyé: " + welcome);
                    ...
                    if (numAbonneStr == null || numAbonneStr.isEmpty()) {
                        ...
                        out.println(error);
                        System.out.println("[ReservationServer] Erreur: " + error);
                        return;
                    }
                    ...
                    out.println(confirm);
                    System.out.println("[ReservationServer] Confirmation: " + confirm);
                    ...
                    // 7. Tenter la réservation
                    try {
                        document.reserver(abonne);
                        Response success = new Response(RequestStatus.SUCCESS, "Réservation effectuée avec succès");
                        out.println(success);
                        System.out.println("[ReservationServer] Succès: " + success);
                    } catch (ReservationException e) {
                        Response error = new Response(RequestStatus.FAILURE, "Erreur de réservation: " + e.getMessage());
                        out.println(error);
                        System.out.println("[ReservationServer] Erreur: " + error);
                    }
                    ...
                    System.out.println("[ReservationServer] Confirmation client reçue: " + confirmation);
                }
            }
        } catch (IOException e) {
            System.err.println("[ReservationServer] Erreur: " + e.getMessage());
        }
    }

    public static void main(String[] args) {
        new ReservationServer().demarrer();
    }
} 